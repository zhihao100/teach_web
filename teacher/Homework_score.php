...
<head>
    ...
    <title>成绩查询-作业成绩</title>
    <link rel="stylesheet" type="text/css" href="CSS/dohomework.css" />
</head>
<body>
<div class="bar"><h3><a href="#">首页</a>&nbsp;&nbsp;&gt;&nbsp;&nbsp;成绩查询-作业</h3></div>
<!-- main-wrap start -->
<div class="main-wrap">
    <div class="select">
    ...
    <div class="info">
        <table border="0px" cellpadding="0px" cellspacing="0px" width="100%" id="showTest" id="table">
            <tr><th>作业名称</th><th>姓名</th><th>班级</th><th>分数</th></tr>
            <?php
           ...
           WHERE test_score.test_id=$test_id and test_score.course_id=$course_id and test_score.class_id=$class_id limit $offset,$num";
            $result_done=mysqli_query($link,$sql_done);
            $score_info=array(array());
            $i=0;
            while($arr=@mysqli_fetch_assoc($result_done)) {
               echo "<tr class='tr2'><td>{$arr['test_title']}</td><td>{$arr['name']}</td><td>{$arr['class_short']}</td><td><span class='wtj'>{$arr['test_score']}</span></td></tr>";
            }?>
        </table>
    </div>
    <div class="page">
    ...
    $(function() {
        ...
        $('#class').on("change",function(){
                ...
                $.ajax({
                    ...
                    data: "act="+"searchFirstScore",
                    success: function (data) {
                        var arr=JSON.parse(data);
                        var i=0;
                        var sum="<tr><th>作业名称</th><th>姓名</th><th>班级</th><th>分数</th></tr>";
                        document.cookie="page_all="+arr[0]['length'];
                        $("table").html(sum);
                        window.location = "Homework_score.php";
                    }
                });
            }
        )
        ...
        $('#test_name').on("change",function(){
                var test_id=$("#test_name").val();
                document.cookie="test_id="+test_id;
                $("table").html("<tr><th>作业名称</th><th>姓名</th><th>班级</th><th>分数</th></tr>");
                $.ajax({
                    datatype: "html",
                    type: "POST",
                    url: "doTeacherAction.php",
                    ...
                    success: function (data) {
                        ...
                        for(i;i<arr.length;i++){
                                sum=sum+" <tr class='tr1'><td>"+arr[i]['test_name']+"</td><td>"+arr[i]['student_name']+"</td><td>"+arr[i]['class_name']+"</td><td><span class='pg'>分数</span></td></tr>";
                            }
                        $("table").html(sum);
                        window.location = "Homework_score.php";
                    }
